...
const mainCards = ORDER.map(slug => {
  ...
  return `  <div class="card" data-item="${slug}">
    <h3>${esc(title)}</h3>
    <div class="chips"><span class="chip">${esc(where)}</span><span class="chip">${p.t0 >= 3600 ? Math.floor(p.t0 / 3600) + ':' + String(Math.floor(p.t0 / 60) % 60).padStart(2, '0') + ':' + String(Math.round(p.t0) % 60).padStart(2, '0') : mmss(p.t0)}</span><span class="chip">${mmss(p.t1 - p.t0)} long</span></div>
    <p class="mini">${esc(gist)}</p>
    <audio controls preload="none" src="${urlFor(slug)}"></audio>
    <div class="passage">${p.paras.map(t => `<p>${esc(t)}</p>`).join('\n      ')}</div>
  </div>`;
}).join('\n');
...
<title>Horns of the Goddess — the magic passages</title>
<link rel="stylesheet" href="/compare.css">
<style>
  .passage { margin-top: 12px; }
...
<div class="wrap">
  <h1>Horns of the Goddess — the magic passages</h1>
...
(function () {
  ...
  window.__compareHelp({ html: '<b>The three at the top are cut.</b> Each one plays its own '
    + 'audio and carries its full text. Everything under them is flagged but not cut yet — '
    + 'leave a note on any of them and I will cut it the same way.' });
})();
</script>`;
...
if (process.argv.includes('--post')) {
  fetch(BASE + '/api/chatfeed/page', {
    ...
    body: JSON.stringify({ chat: CHAT, title: 'Horns of the Goddess — the magic passages', html }),
