// table.js — a TABLE for two people on two phones, anyone's. The one copy.
//
// Lifted out of dominoes.js (2026-09-04) the day Sophie asked for the
// triangle game to be multiplayer too ("how can we make this multiplayer" →
// "turns"): the seats, the invite, the turn gate, the your-turn text and the
// per-player view were already exactly what a second game needed, and a
// second copy of them would drift the first time one game fixed a bug the
// other did not know about. So this is a FACTORY — makeTable() returns a
// router for one game's rooms — and each game's page holds its own rules.
//
...
// A game adds its own routes to the returned router (the Similitude table
// adds the referee and the paid draw) and may pass `extendView(room, me)` to
// ride extra server-owned fields on the read, or async `enrich(room, me)` to
// resolve them first (a drawing card becoming its picture).
//
// makeTable({ collection, page, title, siteEnv?, extendView? })
//   collection — the Firestore collection, one doc per table
//   page       — the page path the seat and invite links point at ('/dominoes')
//   title      — the words in front of every text ('Similitude Dominoes')
// It costs nothing — no model call anywhere; a text is Twilio's own
// fraction of a cent.
'use strict';
...
function makeTable({ collection, page, title, siteEnv, extendView, enrich } = {}) {
  if (!collection || !page || !title) throw new Error('makeTable needs collection, page and title');
  const SITE = (siteEnv && process.env[siteEnv]) || process.env.TABLE_SITE_ORIGIN || 'https://imageforge-q125.onrender.com';
  ...
  const seatLink = (room, p) => `${SITE}${page}?room=${room.id}&t=${room[p].token}`;
  ...
  function notifyPlan(room, me, state, line) {
    ...
    const link = seatLink(room, to);
    return { to, phone: seat.phone, body: `${title} — ${text || (state.over ? 'the round is over.' : 'your turn.')} ${link}` };
  }
  ...
  function view(room, me) {
    ...
    const v = {
      ...
      invite: me === 'a' && !room.b ? `${SITE}${page}?room=${room.id}&join=${room.invite}` : undefined,
      myPhone: room[me] && room[me].phone ? true : false,
    };
    return extendView ? Object.assign(v, extendView(room, me) || {}) : v;
  }

  async function load(id) {
    const snap = await db().collection(collection).doc(String(id || '').slice(0, 40)).get();
    return snap.exists ? snap.data() : null;
  }
  const docRef = (id) => db().collection(collection).doc(String(id || '').slice(0, 40));
  ...
  router.post('/rooms', async (req, res) => {
    try {
      ...
      await db().collection(collection).doc(id).set(room);
      res.json({ ok: true, room: id, token: room.a.token, invite: `${SITE}${page}?room=${id}&join=${room.invite}` });
  ...
  router.post('/rooms/:id/join', async (req, res) => {
    try {
      ...
      await db().collection(collection).doc(room.id).set({ b, line, updatedAt: now() }, { merge: true });
      // the host hears their friend arrived — the one text that is not a move
      if (room.a.phone) sms.sendSms(room.a.phone, `${title} — ${line} ${seatLink(room, 'a')}`).catch(() => {});
  ...
  router.get('/rooms/:id', async (req, res) => {
    try {
      ...
      const v = view(room, me);
      // a game may resolve server-owned fields on the way past (a drawing
      // card becoming its picture) — async, so it rides the route, not view()
      if (enrich) Object.assign(v, (await enrich(room, me)) || {});
      res.json(v);
    } catch (err) { res.status(502).json({ error: err.message }); }
  });
  ...
  router.post('/rooms/:id/move', async (req, res) => {
    try {
      ...
      if (plan) sms.sendSms(plan.phone, plan.body).then((r) => { if (!r.ok && r.reason !== 'not configured') console.warn(title + ': sms', r.reason); }).catch(() => {});
  ...
  router.post('/rooms/:id/phone', async (req, res) => {
    try {
      ...
      await db().collection(collection).doc(room.id).set({ [me]: Object.assign({}, room[me], { phone }) , updatedAt: now() }, { merge: true });
      res.json({ ok: true, phone: !!phone });
    } catch (err) { res.status(502).json({ error: err.message }); }
  });

  return {
    router, init, db, docRef, load, seatLink, SITE,
    _internals: { seatOf, moveGate, notifyPlan, view, cleanName, other },
  };
}

module.exports = { makeTable, seatOf, moveGate, cleanName, other };
