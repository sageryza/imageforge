...
# THE PILL BELT (2026-09-09, Sophie: "add all the pill scenes to ur belt / make sure u get the latest belt w all fixes").
# Built from the SOAP builder — the most complete copy per belt/VERSIONS.md (read-back saver, the 8,000 refusal, the
# redo-notes fold, "what came before", editable reference lines, model's-pick seconds) — plus BELT_CHAT/BELT_KEYS so a
# chat can post its own slice, and SEEDS: her edits already on the soap sheet (43a-e) and the old conveyor sheet seed
# this one; an edit made HERE wins over both.
CHAT=os.environ.get('BELT_CHAT','pull-scenes-risperdal'); SHEET='belt-pills'
KEYS=[k for k in os.environ.get('BELT_KEYS','').split(',') if k]
SEEDS=[('soap-pill-scene','belt-soap'),('severance-api-multiple-frames','conveyor')]
def post(path,body):
    r=urllib.request.Request(B+'/api/chatfeed/'+path,data=json.dumps(body).encode(),headers={'Content-Type':'application/json'})
    return json.load(urllib.request.urlopen(r))
jobs=json.load(open('jobs-pills.json'))
if KEYS: jobs=[j for j in jobs if j['key'] in KEYS]
state=json.load(open('belt-pills-state.json')) if os.path.exists('belt-pills-state.json') else {}
ver=state.get('ver',0)+1
tx=json.load(urllib.request.urlopen(B+'/api/chatfeed/verdict?chat=%s&sheet=%s'%(CHAT,SHEET))).get('texts',{})
for sc,ss in SEEDS:
    seed=json.load(urllib.request.urlopen(B+'/api/chatfeed/verdict?chat=%s&sheet=%s'%(sc,ss))).get('texts',{})
...
function paintPos(){ pos.textContent=(at()+1)+' / '+cards.length; try{localStorage.setItem('beltpills.at',at());}catch(e){} }
...
var h=(location.hash||'').replace('#j-',''); var start=cards.findIndex(function(c){return c.getAttribute('data-key')===h;}); if(start<0){ try{start=parseInt(localStorage.getItem('beltpills.at'))||0;}catch(e){start=0;} }
setTimeout(function(){ deck.scrollLeft=start*deck.clientWidth; paintPos(); window.scrollTo(0,0); },50);
window.__compareNotes({chat:CHAT,sheet:'belt-pills-notes'});
window.__compareHelp({html:'<p class="toc">__TOC__</p><p>Every pill scene in the film, in film order — the one already shot (13, both clips play), the soap scene (43a–e) and the three other pill cards off the big belt, plus the two pill scenes from your notes that never got a card. Your words verbatim in the box. Type <b>cut</b> on a line of its own to split a scene; the line under the box counts the pieces. Fold the box (fold it with the underlined word). Above your words, folded: the reference lines of mine that go in front of them — open the fold to edit them. Under the line is what would go with it: seconds, the videos (playable), the stills. Nothing on this page is sent or wired until you say so.</p>'});
</script>
'''
page=page.replace('__CHAT__',CHAT).replace('__SHEET__',SHEET).replace('__TOC__',TOC).replace('__SECS__','\n'.join(secs))
open('belt-pills.html','w').write(page)
d=post('page',{'chat':CHAT,'title':'The pill scenes v%d'%ver,'html':page}); print(d.get('id'),d.get('warnings'))
for old in state.get('ids',[]): post('page/'+old+'/supersede',{'superseded':True})
json.dump({'ver':ver,'ids':[d['id']]},open('belt-pills-state.json','w'))
