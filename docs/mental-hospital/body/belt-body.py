...
# BELT_CHAT / BELT_KEYS / BELT_TITLE / BELT_SEED let a second chat post its own slice of
# the belt (the climax chat posts md-31a..md-34); the defaults are the montage chat's.
CHAT=os.environ.get('BELT_CHAT','video-editing-continuity'); SHEET='belt-body'
KEYS=[k for k in os.environ.get('BELT_KEYS','').split(',') if k]
TITLE=os.environ.get('BELT_TITLE','The beautiful body — the belt (Mini)')
SEED=os.environ.get('BELT_SEED','')   # a chat whose sheet seeds her edits when this chat has none yet
STATE='belt-body-state.json'
def post(path,body):
    r=urllib.request.Request(B+'/api/chatfeed/'+path,data=json.dumps(body).encode(),headers={'Content-Type':'application/json'})
    return json.load(urllib.request.urlopen(r))
jobs=json.load(open('jobs-body.json'))
...
if SEED:
    seed=json.load(urllib.request.urlopen(B+'/api/chatfeed/verdict?chat=%s&sheet=%s'%(SEED,SHEET))).get('texts',{})
    for k,v in seed.items(): tx.setdefault(k,v)   # her edit elsewhere seeds this chat; this chat's own edit wins
...
for n,j in enumerate(jobs,1):
    ...
    cont=tx.get(k+'.cont') if (k+'.cont') in tx else j.get('cont','')
    contbox=('<details class="text" open><summary>continuity — where we are, what just happened (mine, in brackets, sent first — edit or empty it)</summary><textarea class="p" data-key="%s" data-field="cont" spellcheck="false">%s</textarea><div class="saved" id="sv-%s-cont"></div></details>'%(k,H.escape(cont),k)) if cont else ''
    minebox='<details class="text"><summary>reference lines (mine, sent before your words — edit them)</summary><textarea class="p" data-key="%s" data-field="mine" spellcheck="false">%s</textarea><div class="saved" id="sv-%s-mine"></div></details>'%(k,H.escape(mine),k)
    ...
    sends=' · '.join(['Seedance 2.0 Mini via OpenRouter','480p','3:4','audio on']+['Image%d = %s'%(i+1,H.escape(im[0])) for i,im in enumerate(j['images'])])
    ...
    pending=''.join('<p class="pend">Image%d — %s — NOT MADE YET</p>'%(i+1,H.escape(im[0])) for i,im in enumerate(j['images']) if not im[1])
    stills=''.join('<figure><img loading="lazy" src="%s"><figcaption>Image%d — %s</figcaption></figure>'%(H.escape(im[1]),i+1,H.escape(im[0])) for i,im in enumerate(j['images']) if im[1])
...
%(contbox)s%(minebox)s%(prebox)s<details class="text" open><summary>your words</summary>
...
%(note)s
</div></section>'''%dict(minebox=minebox,prebox=prebox,k=k,n=n,t=H.escape(j['title']),st=H.escape(j['status']),p=H.escape(p),contbox=contbox,s=H.escape(s),sends=sends,films=films,pending=pending,stills=stills,note=note))
...
function cost(k){ var s=parseInt(document.querySelector('.secs[data-key="'+k+'"]').value)||0; document.querySelector('.cost[data-key="'+k+'"]').textContent='about '+Math.round(s*1.39)+'¢ on Mini (1.39¢/s, measured)'; }
...
function paintPos(){ pos.textContent=(at()+1)+' / '+cards.length; try{localStorage.setItem('beltbody.at',at());}catch(e){} }
...
var h=(location.hash||'').replace('#j-',''); var start=cards.findIndex(function(c){return c.getAttribute('data-key')===h;}); if(start<0){ try{start=parseInt(localStorage.getItem('beltbody.at'))||0;}catch(e){start=0;} }
setTimeout(function(){ deck.scrollLeft=start*deck.clientWidth; paintPos(); window.scrollTo(0,0); },50);
window.__compareNotes({chat:CHAT,sheet:'belt-body-notes'});
window.__compareHelp({html:'<p class="toc">__TOC__</p><p>The beautiful body scene (Yolanda and the young male nurse) cut into Mini-sized pieces, film order, your words verbatim in the box. The bracketed continuity box is mine, sent before the reference lines and your words — edit it or empty it. Under the line: seconds, the cost on Mini, the stills that would ride. Nothing on this page is sent until you say go for that exact card.</p>'});
</script>
'''
page=page.replace('__TITLE__',H.escape(TITLE)).replace('__CHAT__',CHAT).replace('__SHEET__',SHEET).replace('__TOC__',TOC).replace('__SECS__','\n'.join(secs))
open('belt-body.html','w').write(page)
