...
window.__compareHelp({html:'<p class="toc">__TOC__</p><p>One card per scene you sent as a file, in film order, your words verbatim in the box (fold it with the underlined word). Above your words, folded: the reference lines of mine that go in front of them — open the fold to edit them. Under the line is what would go with it: seconds, the videos (playable), the stills. Nothing on this page is sent or wired until you say so.</p>'});
</script>
'''
