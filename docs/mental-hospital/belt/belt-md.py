...
CHAT=os.environ.get('BELT_CHAT','hospital-night-film'); SHEET='belt-md'
KEYS=[k for k in os.environ.get('BELT_KEYS','').split(',') if k]
TITLE=os.environ.get('BELT_TITLE','Her scenes — the belt')
SEED=os.environ.get('BELT_SEED','')   # a chat whose sheet seeds her edits when this chat has none yet
STATE='belt-md-state.json' if CHAT=='hospital-night-film' else 'belt-md-%s-state.json'%CHAT
def post(path,body):
