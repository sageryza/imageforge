...
CHAT=os.environ.get('CHAT','hospital-night-film')   # the chat the clip pins and files into
...
pin=json.dumps({"chat":CHAT,"session":S,"url":it['url'],"title":"Newest clip · "+title,"kind":"video"}).encode()
...
try:
    ...
    urllib.request.urlopen(urllib.request.Request(B+'/api/chatfeed/clips',data=json.dumps({'chat':CHAT,'session':S,'url':it['url'],'title':title,'id':it['id']}).encode(),headers={'content-type':'application/json'}))
    urllib.request.urlopen(urllib.request.Request(B+'/api/deliverables',data=json.dumps({'chat':CHAT,'session':S,'url':it['url'],'title':title,'kind':'video'}).encode(),headers={'content-type':'application/json'}))
