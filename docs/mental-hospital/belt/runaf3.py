...
body={"prompt":open(pfile).read().rstrip('\n'),"model":"seedance-2.5","resolution":"480p","duration":int(os.environ.get("DUR","30")),"generateAudio":True,"aspectRatio":os.environ.get("AR","3:4"),"referenceVideoUrls":json.loads(os.environ.get("REFS","[]")),"referenceImageUrls":json.loads(os.environ.get("IMGS","[]")),"chat":"hospital-night-film","scene":key,"title":title,"session":S}
...
pin=json.dumps({"chat":"hospital-night-film","session":S,"url":it['url'],"title":"Newest clip · "+title,"kind":"video"}).encode()
...
try:
    ...
    urllib.request.urlopen(urllib.request.Request(B+'/api/chatfeed/clips',data=json.dumps({'chat':'hospital-night-film','session':S,'url':it['url'],'title':title,'id':it['id']}).encode(),headers={'content-type':'application/json'}))
    urllib.request.urlopen(urllib.request.Request(B+'/api/deliverables',data=json.dumps({'chat':'hospital-night-film','session':S,'url':it['url'],'title':title,'kind':'video'}).encode(),headers={'content-type':'application/json'}))
