...
B='https://imageforge-q125.onrender.com'; CHAT='hospital-severance-rough-cut'; SHEET='belt-48'
def post(path,body):
    r=urllib.request.Request(B+'/api/chatfeed/'+path,data=json.dumps(body).encode(),headers={'Content-Type':'application/json'})
    return json.load(urllib.request.urlopen(r))
jobs=json.load(open('jobs-48.json'))
state=json.load(open('belt-48-state.json')) if os.path.exists('belt-48-state.json') else {}
...
function paintPos(){ pos.textContent=(at()+1)+' / '+cards.length; try{localStorage.setItem('belt48.at',at());}catch(e){} }
