...
const OUT=path.join(__dirname,'out6');fs.mkdirSync(OUT,{recursive:true});
...
This is not a scene. Draw one single subject from the moment—the one object or figure that carries its meaning—surrounded by generous empty white space, the way a single sticker sits alone on a page. Plain white background. No setting, no environment, no scenery, no ground, no background elements or props around it. Keep it simple, uncluttered, and readable at a glance.
