...
/// into sentence-level lines she can split, meld, mark, reorder, respeak and
/// hear as marked before anything is cut for real. Wraps the server's
/// gated /blocks page (public/blocks.html, engine at /api/blocks in
/// blocks.js) exactly like the Episode Editor wraps /editor: native tool
...
struct BlocksView: View {
    ...
    @StateObject private var webRef = BlocksWebRef()
    ...
    var body: some View {
        Group {
            if loadFailed {
                VStack(spacing: 14) {
                    ...
                    Text("Couldn't open Cutting Blocks")
                        .font(.headline)
                ...
                BlocksWebView(token: studioToken, failed: $loadFailed, webRef: webRef, onLeave: goBack)
                    .id(reloadKey)
        ...
        .forgeWebToolBar("Cutting Blocks", tint: Self.ink, paper: Self.paper, failed: loadFailed, back: navBack)
    }
...
final class BlocksWebRef: ObservableObject { weak var web: WKWebView? }
...
private struct BlocksWebView: UIViewRepresentable {
    let token: String
    @Binding var failed: Bool
    let webRef: BlocksWebRef
    ...
    func makeUIView(context: Context) -> WKWebView {
        ...
        webRef.web = web
        if let url = URL(string: MovieService.serverURL + "/blocks") {
    ...
    final class Coordinator: NSObject, WKNavigationDelegate, WKUIDelegate {
        ...
        let parent: BlocksWebView
        private var screenChangeObserver: NSObjectProtocol?
        init(_ parent: BlocksWebView) { self.parent = parent }
        ...
        // The /blocks page sits behind HTTP Basic (any user, password = token).
