...
/// Pausing — how long a beat sits, the other half of the polish pass. The
/// Cutting Room can only REMOVE a pause (compressed to ~0.28s); here she sets
/// a length, adds a pause where the recording has none, and hears her EDIT
/// rather than the source — every pause built out of the recording's own room
/// tone, never digital silence. Wraps the server's
/// gated /pausing page (public/pausing.html, engine at /api/pausing in
/// pausing.js) exactly like the Episode Editor wraps /editor: native tool
...
struct PausingView: View {
    ...
    @StateObject private var webRef = PausingWebRef()
...
private struct PausingWebView: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> WKWebView {
        ...
        webRef.web = web
        if let url = URL(string: MovieService.serverURL + "/pausing") {
    ...
    final class Coordinator: NSObject, WKNavigationDelegate, WKUIDelegate {
        ...
        // The /pausing page sits behind HTTP Basic (any user, password = token).
