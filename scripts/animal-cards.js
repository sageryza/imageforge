...
// --name "Fruit deck" --slug fruit: the same card and zip for ANOTHER deck
// (2026-09-26, Sophie: "fruit and animals shud be same layout spacing?") —
// the title, the Dump album, the sheet and the record file wear the name.
const NAME = flag('name', 'Animal deck'), SLUG = flag('slug', 'animals');
...
const upload = async (buf, filename, ct, media) => {
  const q = new URLSearchParams({ session: 'animal-cards', bundle: `${SLUG} cards v${VERSION}`, filename, from: 'claude', ...(media ? { media } : {}) });
...
(async () => {
  ...
  const deck = path.join(OUT, `${SLUG}-v${VERSION}`);
  ...
  const zipPath = path.join(OUT, `${SLUG}-cards-v${VERSION}.zip`);
  fs.rmSync(zipPath, { force: true });
  execFileSync('zip', ['-q', '-r', zipPath, '.'], { cwd: deck });
  const zip = await upload(fs.readFileSync(zipPath), `${SLUG}-cards-v${VERSION}.zip`, 'application/zip', 'file');
  ...
  for (let i = 0; i < items.length; i += 2) {
    rows.push(`<div class="imgrow">${items.slice(i, i + 2).map((it) => `<figure data-item="${SLUG}-${esc(it.file.replace(/\.png$/, ''))}"><img src="${esc(it.url)}" alt="${esc(it.name)}"><figcaption>${esc(it.name)}</figcaption></figure>`).join('')}</div>`);
  }
  const title = `${NAME} — ${items.length} cards, MPC print files v${VERSION}`;
...
<div class="wrap">
  <h1>${esc(title)}</h1>
  <p class="save"><a href="${save}">save the ${NAME.toLowerCase()} (zip, ${mb}MB)</a></p>
  ${LEAD ? fs.readFileSync(LEAD, 'utf8') : ''}
  ${rows.join('\n')}
</div>
<script src="/compare.js"></script>
<script>(function(){ if (window.__compareHelp) window.__compareHelp({ text: ${JSON.stringify(help)} }); if (window.__compareNotes) window.__compareNotes({ chat: ${JSON.stringify(CHAT)}, sheet: '${SLUG}-cards-v${VERSION}' }); })();</script>
`;
  ...
  fs.writeFileSync(path.join(__dirname, 'decks', `${SLUG}-cards-v${VERSION}.json`), JSON.stringify({ page: posted.id, zip: save, items }, null, 1));
