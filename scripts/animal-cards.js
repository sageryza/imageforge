...
const recs = JSON.parse(fs.readFileSync(REC, 'utf8')).sort((a, b) => a.name.localeCompare(b.name));
console.log(`${recs.length} cards · ${path.basename(FONT)} · v${VERSION}`);
if (DRY) { for (const r of recs) console.log(`  ${r.name} ← ${r.full}`); process.exit(0); }

const esc = (s) => String(s).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/"/g, '&quot;');
const upload = async (buf, filename, ct, media) => {
  const q = new URLSearchParams({ session: 'animal-cards', bundle: `animal cards v${VERSION}`, filename, ...(media ? { media } : {}) });
...
(async () => {
  ...
  execFileSync('python3', [path.join(__dirname, 'flashcard-compose.py'), path.join(OUT, 'cards.txt'), '--src', full, '--full', full, '--font', FONT, '--out', deck], { stdio: 'inherit' });
  ...
  const title = `Animal flash cards — ${items.length}, print files v${VERSION}`;
...
<script>(function(){ if (window.__compareNotes) window.__compareNotes({ chat: ${JSON.stringify(CHAT)}, sheet: 'animal-cards-v${VERSION}' }); })();</script>
`;
