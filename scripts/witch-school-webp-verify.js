#!/usr/bin/env node
/**
 * Guard for the webp switch: every school image witch.html asks for must exist
 * in witch-school/webp/ before the page is allowed to point there. A missing
...
(async () => {
  ...
  const [pngs] = await bucket.getFiles({ prefix: 'witch-school/assets/' });
  const havePng = new Set(pngs.filter(f => f.name.endsWith('.png')).map(f => f.name.replace('witch-school/assets/', '').replace(/\.png$/, '')));
