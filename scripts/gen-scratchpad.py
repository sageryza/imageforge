...
<!-- THE CAST RULES, linked and never copied (/pad-characters.js): which
     picked character IS Sophie, so the house card's button can say it is
     standing down for her own (2026-09-06, "it used the watercolor
     reference not the blue pajamas i added"). The draw's own rule is
     houseCardRides in that file; the page only paints it. -->
...
/* her OWN Sophie is picked — the house card stands down for that draw */
#dchar.on.stood{opacity:.3; border-color:var(--line);}
...
function paintDchars(){
  ...
  /* ONE Sophie per draw: a picked character who IS her (the shared rule,
     never a copy of the name list) stands the house card down, and the
     button says so rather than lying lit. The draw's own rule is
     houseCardRides in pad-characters.js; this only paints it. */
  var pc=window.__padCharacters;
  var self=!!pc&&pickedChars.some(function(id){
    var c=padChars.find(function(x){return x.id===id;}); return !!(c&&pc.isSelf(c.name));
  });
  var dc=document.getElementById('dchar');
  dc.classList.toggle('stood',self);
  dc.title=self?'Your own Sophie is picked — this card stands down for the draw':'';
}
