...
#pad{display:grid; grid-template-columns:repeat(3,1fr); gap:14px; margin-top:1.4em;}
.beat{position:relative; aspect-ratio:2/3; border:1.5px solid var(--line); border-radius:4px;
...
.slot{aspect-ratio:2/3; border:1.5px dashed var(--ink2); border-radius:4px; background:none; padding:0; cursor:pointer;}
/* ── overlays ─────────────────────────────────────────────────────── */
...
#beatpop{position:fixed; inset:0; z-index:50; display:flex; flex-direction:column; align-items:center;
  justify-content:center; gap:22px; background:rgba(20,17,12,.82); padding:5vw;}
#beatpop img{max-width:88vw; max-height:62vh; border:3px solid var(--line); border-radius:4px; background:var(--barbg);}
...
/* ── the beat popup: the picture big, and its frame color ─────────── */
function openBeat(b){
  popBeat=b;
...
function closeBeat(){document.getElementById('beatpop').hidden=true; popBeat=null; lock(false); render();}
