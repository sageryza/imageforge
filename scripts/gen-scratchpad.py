...
/* The floating autoscroll pill is fixed over the top-right corner, so the
   header row stops 56px short of it — the buttons sit left of that. The word
   SCRATCH PAD is the header, centered across the very top; the buttons ride
   the same row at its right. */
header{display:flex; align-items:center; justify-content:flex-end; gap:10px; padding-right:56px; min-height:34px; position:relative;}
header .no{position:absolute; left:0; right:0; text-align:center; pointer-events:none;}
...
/* An empty beat's popup: same blank paper tile, and the Playground shortcut
   (the in-popup art generator comes later). */
...
#pgo{font-family:'EBGaramond',Georgia,serif; font-size:15px; color:#fff; background:none;
  border:1.5px solid rgba(255,255,255,.55); border-radius:6px; padding:8px 14px; cursor:pointer;}
...
<div id="beatpop" hidden>
  ...
  <button id="pgo" hidden>Open the Playground</button>
</div>
...
function openBeat(b){
  ...
  document.getElementById('pgo').hidden=Boolean(b.url);
...
document.getElementById('pgo').onclick=function(ev){
  ev.stopPropagation();
