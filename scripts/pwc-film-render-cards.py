...
for fname, clip, lead, tail, shotlist in shots.CARDS:
    ...
    U0 = fw/cw
    # Beats were MEASURED on the old (slower) clip, so a position maps to the
    # new clip by new/old — v6 and v7 had this inverted (old/new), which pushed
    # every camera move and every ink mark ~18% LATE in clip time: the mirror
    # was still circled on "attempt the spoon", and the spoon circle, the
    # "absolutely nothing" arrow and card 7's final pull-out fell off the end
    # of their cards entirely. Sophie's "the circles weren't making sense with
    # the story" was this line.
    ratio = vod["new"][clip] / vod["old"][clip]
