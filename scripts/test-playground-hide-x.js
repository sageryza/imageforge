...
(async () => {
  ...
  // THE TWO MARKS ARE BACK ON THE FEED BAR (2026-09-02, Sophie: "actually put
  // the heart x thing exactly where it was"). They rode inside the filters
  // drawer for a day; that is history, not a rule.
  const HEART = '#v-liked';
  const NOX = '#v-hidex';
  ...
  const colors = await page.evaluate(() => {
    const h = document.getElementById('v-liked');
    const x = document.getElementById('v-hidex');
