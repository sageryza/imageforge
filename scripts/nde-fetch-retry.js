#!/usr/bin/env node
/**
 * nde-fetch-retry.js — retry the 14 videos that failed the first pass.
 * Slower (dodges YouTube rate-limiting), retries within yt-dlp, and prints the
 * REAL reason on any failure so we can see what's genuinely uncaptioned.
 * Same as before: needs yt-dlp, no keys. Run:  node ~/Downloads/nde-fetch-retry.js
 */
...
(async () => {
  console.log(`Retrying ${VIDEOS.length} videos (slow mode) → ${SERVER}\n`);
  ...
  for (let i = 0; i < VIDEOS.length; i++) {
    ...
    await new Promise(r => setTimeout(r, 3000)); // extra pause between videos
  }
