...
for path, st, fo in BED:
    ...
    fc.append(f"[{i}:a]aresample=44100,aformat=channel_layouts=stereo,volume=0.075,"
              ...
              f"adelay={int(st*1000)}|{int(st*1000)}[v{i}]"); mixl.append(f"[v{i}]")
# the FAIL stamp lands with a thud — her note: "a fail sound effect and stamp
# the fail on yourself". The picture takes a jolt at the same instant.
i = add("mus/stamp.wav")
fc.append(f"[{i}:a]aresample=44100,aformat=channel_layouts=stereo,volume=0.85,"
          f"adelay={int(T['fail']*1000)}|{int(T['fail']*1000)}[v{i}]"); mixl.append(f"[v{i}]")
