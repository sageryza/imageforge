...
async function pageHalf() {
  ...
  const chipY = await page.$eval('#kinds', el => el.getBoundingClientRect().top);
  ok(chipY > 0 && chipY < 300, 'the kind chips sit at the top of the list (y=' + Math.round(chipY) + ')');
  await page.click('#kinds .chip[data-k=fruit]'); await page.waitForTimeout(100);
  ok((await page.$$eval('.pc', els => els.map(e => e.dataset.id).join())) === 'pear' && (await page.$$eval('.kindh', els => els.length)) === 0, 'a kind chip shows only that kind, with no heading over it');
  await page.click('#kinds .chip[data-k=""]'); await page.waitForTimeout(100);
  ok((await page.$$eval('.pc', els => els.length)) === 3 && (await page.$$eval('.kindh', els => els.length)) === 3, '"all" brings every kind back under its heading');
  ok((await page.$$eval('.kindh', els => els.map(e => e.textContent).join())) === 'animals,fruits,vegetables', 'grouped by kind');
  await shot('1-pieces');
  await page.$eval('.pc[data-id=bear]', el => { el.__mark = 1; });
  ok(await page.$eval('#shelf', el => el.hasAttribute('data-nostop')), 'the list is marked so a tick never starts the autoscroll');
  ...
  ok(await page.$eval('.pc[data-id=bear]', el => el.__mark === 1), 'the tick toggled the tile that was already there — no rebuild, no flash');
  ...
  await page.click('.pc[data-id=bear]'); await page.waitForTimeout(700);
  ok((await page.$$eval('.pc.on', els => els.length)) === 1, 'an untick on PIECES…');
  await page.click('#undo'); await page.waitForTimeout(700);
  ok((await page.$$eval('.pc.on', els => els.map(e => e.dataset.id).join())) === 'bear,pear' && store().items.length === 2, '…comes back with undo from the same row, on this tab too');
