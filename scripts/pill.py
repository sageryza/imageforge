...
PILL_JS = """
var SPEEDS=[['Slow',0.5],['Medium',1.0],['Fast',1.9],['Fastest',3.2]];
...
// while autoscrolling, a SINGLE tap stops it and a DOUBLE tap bumps it one
// speed faster. Starting is always via the play button, never a tap — so a tap
// on a stopped page does nothing here.
var _tapT=null;
window.__scrollFaster=function(){ if(playing){ si=Math.min(SPEEDS.length-1, si+1); paintPill(); } };
window.__scrollTap=function(){
  if(!playing) return;
  if(_tapT){ clearTimeout(_tapT); _tapT=null; window.__scrollFaster(); }   // double → faster
  else { _tapT=setTimeout(function(){ _tapT=null; scrollStop(); }, 280); } // single → stop
};
