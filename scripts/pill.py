...
// while autoscrolling, EVERY tap steps the speed up one notch — and once it's
// already on the top speed, the next tap pauses. Immediate, no double-tap
// waiting. Starting is always via the play button, never a tap.
window.__scrollTap=function(){
  if(!playing) return;
  if(si<SPEEDS.length-1){ si++; paintPill(); }
  else scrollStop();
};
