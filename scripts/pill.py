...
// A RESUMED SCROLL GOES THE WAY IT WAS GOING (Aug 2026, Sophie: "I can't
// scroll back up only down"). This used to be a hardcoded scrollStart(1), so
// riding UP the page and tapping to pause left her one tap from going back
// DOWN — the ▲ had to be found again for every single pause. `dir` is the
// same memory `vmid` has always used; the two now agree.
window.__scrollTap=function(e){
  if(e && pillInteractive(e.target||e.srcElement)) return;
  playing? scrollStop() : scrollStart(dir||1);
};
...
function syncPtop(){
  if(!_ptop) return;
  _ptop.classList.toggle('on', window.scrollY > window.innerHeight && pageScrolls());
}
