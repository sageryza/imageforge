...
const CHATS = {
  ...
  // `research` and not `built`: the old two-field shape is what this chat is
  // for, and `built` became archive-only in Aug 2026, so it is no longer a chip
  // on the home row to filter by.
  'both-ways': { account: '1', lastSeen: MSGS[0].created, category: 'witch', tags: ['research'], filedAt: iso(T0 - 500) },
...
async function pageTests() {
  ...
  try {
    ...
    ok('…and the old tag words alongside them',
      chips.indexOf('Images') > -1 && chips.indexOf('Research') > -1, chips.join(' · '));
    ...
    await page.click('#thread .orgbtn');
    await page.waitForSelector('.askwrap .arctags');
    // `Images` and not `Bug fix`: the four outcome words are offered in the
    ...
    ok('her folders arrive lit', pileLit.indexOf('Witch') > -1 && pileLit.indexOf('Story') > -1,
      pileLit.join(','));
    ok('…and the tag words do not', pileLit.indexOf('Images') < 0, pileLit.join(','));
    await (await page.$(top + ' .arctags button:text-is("Images")')).click();
    await page.waitForTimeout(250);
    const pileSaved = posts[posts.length - 1];
    ok('flipping a word posts the flip, not the chat',
      pileSaved && pileSaved.label === 'images' && pileSaved.pile === true,
    ...
    // `research` is a CATEGORY word, so it lives behind SEE MORE — the row opens
    ...
    ok('a legacy tag word shows as a chip on the home row now it is in use',
      folders.indexOf('Research') > -1, folders.join(' · '));
    ...
    await page.$$eval('#catrow .catchip', (ns) => {
      const b = ns.find((n) => n.firstChild && n.firstChild.textContent.trim() === 'Research'); if (b) b.click();
    });
    await page.waitForTimeout(200);
