...
// the MIDDLE, then a closing bit for Sophie. foldBody keeps the opening and
// closing prose visible and tucks the technical middle behind a tap — nothing
// deleted, one tap to reveal. Conservative on purpose: only folds when there's
// prose on BOTH sides, and never folds a block containing a link, so her
// closing message + testing links always stay in view.
...
function isTech(b){
  ...
  if(/^```/.test(t)) return true;                 // fenced code = always technical
  if(/https?:\/\//.test(t)) return false;         // a link → it's a message for her
  ...
  for(var i=0;i<lines.length;i++){
    ...
    if(/^\s*(\$|>|git|npm|npx|node|python3?|cd|curl|sudo|ls|cat|grep|mkdir|rm|cp|mv|chmod|brew|xcrun|pip|bash|sed|awk|echo|touch)\b/.test(l)
      || /^[+\-]\s/.test(l)                        // diff line
      || /\b[0-9a-f]{7,40}\b/.test(l)              // commit hash / id
      || /\s\/[\w.-]+\/[\w./-]+/.test(l)           // absolute-ish path
      || /[\w-]+\.(js|ts|tsx|swift|py|json|css|html|yml|yaml|sh|xml|md|plist|xcassets|svg)\b/.test(l)
    ) hits++;
  }
...
function foldBody(text){
  ...
  var tech=blocks.map(isTech);
  var first=tech.indexOf(true), last=tech.lastIndexOf(true);
  // Need technical content bracketed by prose on both sides, else show it all.
  if(first<1 || last>=blocks.length-1 || last<first) return md(text);
  var mid=blocks.slice(first, last+1).join('\n\n');
  if(mid.replace(/\s/g,'').length<140) return md(text);   // too small to bother
  var head=blocks.slice(0, first).join('\n\n');
  var tail=blocks.slice(last+1).join('\n\n');
  return md(head)
    + '<div class="fold"><button type="button" class="foldtog" aria-expanded="false">··· working details</button>'
    + '<div class="foldbody" hidden>'+md(mid)+'</div></div>'
    + md(tail);
}
...
function renderMsg(m){
  ...
  // Wire the "working details" fold toggle (if this message has one). Stops
  // propagation so tapping it doesn't also trigger the reading-aid autoscroll.
  var ft=row.querySelector('.foldtog');
  if(ft){ ft.onclick=function(e){ e.stopPropagation();
    ...
    api('/api/gallery/assets?chat='+encodeURIComponent(name)+'&limit=300')
      ...
      .then(function(d){
        ...
        la.onclick=function(){
          ...
          function tick(){
            if(!q.length && !active){ la.textContent='All loaded'; setTimeout(function(){ la.style.visibility='hidden'; },1200); return; }
        ...
        arow.appendChild(seg); arow.appendChild(la);
