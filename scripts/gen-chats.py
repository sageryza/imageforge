...
function sortedChatNames(g, mode){
  var names=Object.keys(g);
  Object.keys(chats).forEach(function(n){ if(names.indexOf(n)<0) names.push(n); });
  if(mode==='list'){
    // "in order of which one came first" — oldest chat first, stable
    names.sort(function(a,b){
      var fa=(g[a]&&g[a][0]&&g[a][0].created)||'\\uffff', fb=(g[b]&&g[b][0]&&g[b][0].created)||'\\uffff';
      return fa<fb?-1:1;
    });
  } else {
    names.sort(function(a,b){
...
function renderHome(){
  ...
  var g=groups();
  var names=sortedChatNames(g, view);
