...
.shelfrow{display:grid; grid-template-columns:repeat(3,1fr); gap:20px 14px; align-items:end;}
...
function openProj(p, jumpBeat){
  ...
  var voRec=null, voRecording=false;
  function drawVo(){
    voWrap.innerHTML='';
    var vo=p.voiceover||{};
    ...
    var rec=document.createElement('button'); rec.className='btn'+(voRecording?' primary':'');
    rec.textContent= voRecording? '\\u25a0 Stop recording':'Record';
    rec.onclick=function(){
      if(voRecording){ voRec&&voRec.stop(); return; }
      if(!navigator.mediaDevices||!navigator.mediaDevices.getUserMedia||typeof MediaRecorder==='undefined'){ toast('Mic not available here'); return; }
      navigator.mediaDevices.getUserMedia({audio:true}).then(function(stream){
        var chunks=[]; voRec=new MediaRecorder(stream);
        voRec.ondataavailable=function(e){ if(e.data.size) chunks.push(e.data); };
        voRec.onstop=function(){
          stream.getTracks().forEach(function(t){t.stop()});
          voRecording=false;
          var blob=new Blob(chunks,{type:voRec.mimeType||'audio/webm'});
          if(blob.size>30000000){ toast('That recording is too big'); drawVo(); return; }
          var r=new FileReader();
          r.onload=function(){ postVo({audio:r.result}, 'Uploading the voiceover\\u2026'); };
          r.readAsDataURL(blob);
        };
        voRec.start(); voRecording=true; drawVo();
      }, function(){ toast('Mic not available here'); });
    };
    var up=document.createElement('button'); up.className='btn'; up.textContent='Upload audio';
    up.onclick=function(){
    ...
    row.appendChild(rec); row.appendChild(up); row.appendChild(sc);
