...
.row{display:flex; gap:14px; align-items:center; width:100%; text-align:left; background:none; border:none; border-bottom:1px solid var(--line);
  padding:16px 2px; cursor:pointer; color:var(--ink); font-family:'EBGaramond',Georgia,serif;}
.row:focus-visible{outline:2px solid var(--rose);}
.r-cover{width:52px; height:70px; object-fit:cover; border-radius:4px; background:var(--line); flex:none;}
.r-cover.ph{display:flex; align-items:center; justify-content:center; color:var(--ink2); font-size:1.3em;}
.r-body{flex:1; display:flex; flex-direction:column; gap:2px;}
.r-name{font-size:1.35em; font-weight:600;}
.r-meta{font-family:-apple-system,sans-serif; font-size:10px; letter-spacing:.12em; color:var(--ink2); text-transform:uppercase;}
...
function renderShelf(){
  ...
  projects.forEach(function(p){
    ...
    b.innerHTML=(p.cover? '<img class="r-cover" alt="" src="'+esc(p.cover)+'">'
                        : '<div class="r-cover ph">?</div>')
      +'<span class="r-body"><span class="r-name">'+esc(p.title||p.id)+'</span>'
      +'<span class="r-meta">'+beats+(beats===1?' beat':' beats')+'</span>'
