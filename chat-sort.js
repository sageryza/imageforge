...
BUG FIX IS THE LENIENT ONE, and what it really measures is TURNAROUND — how quickly the work gets done and how soon she can archive the chat. Small, bounded work that lands in a turn or two is a bug fix even when nothing was strictly broken: filling a gap in an existing surface (a shape its siblings already had, a control one page has and another is missing, behaviour that does not match its iOS or web twin, something a restructure lost), a tweak, an adjustment, a repair. Adding a square story type where other story shapes already existed is a BUG FIX — it should have been there, and it is done and archivable fast. Reserve "new feature" for genuinely NEW capability with real scope — a surface, tool or behaviour being built rather than completed, the kind of chat that stays open a while.
